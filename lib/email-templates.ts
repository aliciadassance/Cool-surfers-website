...
export const formatPackage = (value?: string) => (value ? PACKAGE_LABELS[value] || value : undefined);

function detailsTable(rows: [string, string | undefined][]) {
  const cells = rows
    .filter(([, value]) => !!value)
    .map(
      ([label, value]) => `
...
function shell(opts: { preheader: string; eyebrow: string; heading: string; bodyHtml: string }) {
            ...
            <tr>
              <td style="background:${COLORS.coconut};padding:24px 40px;text-align:center;">
                <p style="margin:0 0 8px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:12px;color:${COLORS.husk};">
                  Tamraght, Taghazout Bay · Agadir, Morocco
                </p>
